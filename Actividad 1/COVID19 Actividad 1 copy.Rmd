---
title: "Actividad 1"
author: "Ana Valeria García A00826895"
date: "2023-04-12"
output: 
  html_document:
    toc: true
...
```{r, include= FALSE}
setwd("/Users/anavaleriagarciarenteria/Desktop//spda_covid19")
dir()
```
...
map<-readShapePoly("/Users/anavaleriagarciarenteria/Desktop//spda_covid19/shp_mx_mpios/mx_mpios.shp",IDvar="IDUNICO",proj4string=CRS("+proj=longlat")) ### reads data from a polygon shapefile.
...
```{r}
map_sf<-read_sf("/Users/anavaleriagarciarenteria/Desktop//spda_covid19/shp_mx_mpios/mx_mpios.shp")
```
 ...
 *DE AQUI PARA ABAJO NO CORRIÓ EL CÓDIGO, error en el código de queen*
 
```{r}
map_mpios_mx<-st_read("/Users/anavaleriagarciarenteria/Desktop//spda_covid19/avgr/avgr_shp/mx_mpios_data.shp") 
```

```{r}
# crear un objeto de matriz de ponderaciones espaciales (caso de contigüidad) según GEODA
queen_w<-queen_weights(map_mpios_mx)
```

```{r}
# Calcular indicadores locales de autocorrelación espacial - LISA según GEODA
lisa_covid2021<-local_moran(queen_w, map_mpios_mx["dic_2021"]) 
lisa_ppp<-local_moran(queen_w, map_mpios_mx["porcentaje_pob_pobreza"]) 
lisa_gini<-local_moran(queen_w, map_mpios_mx["gin2015"]) 
lisa_n<-local_moran(queen_w, map_mpios_mx["n"]) 
lisa_p2022<-local_moran(queen_w, map_mpios_mx["poblacion_2022"]) 
lisa_ppass<-local_moran(queen_w, map_mpios_mx["porcentaje_pob_acceso_ss"]) 
```

```{r}
# Agregue cálculos de conglomerados al conjunto de datos del mapa
map_mpios_mx$cluster_covid2021<-as.factor(lisa_covid2021$GetClusterIndicators())
levels(map_mpios_mx$cluster_covid2021)<-lisa_covid2021$GetLabels() ### clusters para confirmados de covid dic 2021
```

```{r}
map_mpios_mx$cluster_ppp<-as.factor(lisa_ppp$GetClusterIndicators())
levels(map_mpios_mx$cluster_ppp)<-lisa_ppp$GetLabels() ### clusters para porcentaje de poblacion en pobreza
```

```{r}
map_mpios_mx$cluster_gini<-as.factor(lisa_gini$GetClusterIndicators())
levels(map_mpios_mx$cluster_gini)<-lisa_gini$GetLabels() ### clusters para gini index in 2015
```

```{r}
map_mpios_mx$cluster_n<-as.factor(lisa_n$GetClusterIndicators())
levels(map_mpios_mx$cluster_n)<-lisa_n$GetLabels() ### clusters para numero de hospitales por municipio
```

```{r}
map_mpios_mx$cluster_p2022<-as.factor(lisa_p2022$GetClusterIndicators())
levels(map_mpios_mx$cluster_p2022)<-lisa_p2022$GetLabels() ### clusters para población 2022
```

```{r}
map_mpios_mx$cluster_ppass<-as.factor(lisa_ppass$GetClusterIndicators())
levels(map_mpios_mx$cluster_ppass)<-lisa_ppass$GetLabels() ### clusters para porcentaje de población con acesso a servicios de salud
```

# Clusters

### **Vizualizando Clusters de las variables seleccionadas usando GeoDa View**

```{r}
ggplot(data=map_mpios_mx) + 
  geom_sf(aes(fill=cluster_covid2021)) + 
  ggtitle(label="COVID 19 in dic 2021", subtitle="Confirmed Cases")
```
```{r}
ggplot(data=map_mpios_mx) + 
  geom_sf(aes(fill=cluster_ppp)) + 
  ggtitle(label="% de población en pobreza", subtitle="Población en pobreza")
```
```{r}
ggplot(data=map_mpios_mx) +
...
```{r}
ggplot(data=map_mpios_mx) + 
  geom_sf(aes(fill=cluster_p2022)) + 
  ggtitle(label="Poblacion 2022", subtitle="Población por municipio en 2022")
```
```{r}
ggplot(data=map_mpios_mx) + 
  geom_sf(aes(fill=cluster_ppass)) + 
  ggtitle(label="% de población con acceso a servicios de salud", subtitle="% de población con acceso a servicios de salud")
```
